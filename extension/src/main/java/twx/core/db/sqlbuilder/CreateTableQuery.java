...
import twx.core.db.sqlbuilder.custom.HookAnchor;
import twx.core.db.sqlbuilder.custom.oracle.OraTableSpaceClause;
...
{
  /**
   * The HookAnchors supported for CREATE TABLE queries.  See {@link twx.core.db.sqlbuilder.custom}
  ...
  public CreateTableQuery(Table table, boolean includeColumns) {
    ...
    if(includeColumns) {
      ...
      _constraints.addObjects(Converter.CUSTOM_TO_CONSTRAINTCLAUSE, table.getConstraints());
    }
  }
  ...
  /**
   * Adds custom SQL to this query.  See {@link twx.core.db.sqlbuilder.custom} for more details on
   * custom SQL syntax.
